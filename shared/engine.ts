import { ADJACENCY, CASTLE_MAP, NODE_BY_ID, edgeKey } from './map';
...
import type { GameAction, GamePlayer, GameRules, GameState, GameView, PendingEffect, Troop, TroopType, WinReason } from './types';
...
export function createGame(playerIds: [string, string], rng: Random = secureRandom, rules: GameRules = DEFAULT_RULES): GameState {
  ...
  const game: GameState = {
    ...
    players,
    board: Object.fromEntries(CASTLE_MAP.nodes.map((node) => [node.id, []])),
    cutEdges: [],
    claimedRegions: Object.fromEntries(CASTLE_MAP.regions.map((region) => [region.id, null])),
    discarded: [],
...
export function neighbors(game: Pick<GameState, 'cutEdges'>, nodeId: string): string[] {
  const cut = game.cutEdges;
  return cut.length ? ADJACENCY[nodeId]!.filter((other) => !cut.includes(edgeKey(nodeId, other))) : ADJACENCY[nodeId]!;
}
...
export function connectedNodes(game: GameState, playerId: string): Set<string> {
  ...
  const hq = CASTLE_MAP.nodes.find((node) => node.kind === 'hq' && node.ownerIndex === index)!;
  const connected = new Set([hq.id]);
  const queue = [hq.id];
...
export function canPlaceTroop(game: GameState, playerId: string, troop: Troop, nodeId: string): boolean {
  ...
  if (occupant && occupant.ownerId !== playerId && troop.type !== 'duck' && occupant.type !== 'duck'
      && attackPower(game, playerId, troop, nodeId) <= TROOPS[occupant.type].power) return false;
  if (troop.type === 'pirate' && node.kind !== 'hq') return true;
  if (troop.type === 'ninja' && node.kind !== 'hq' && neighbors(game, nodeId).some((other) => NODE_BY_ID[other]!.kind === 'hq' && NODE_BY_ID[other]!.ownerIndex !== ownIndex)) return true;
...
export function attackPower(game: GameState, playerId: string, troop: Troop, nodeId: string): number {
  const base = TROOPS[troop.type].power;
  if (!game.rules.flank) return base;
  return base + neighbors(game, nodeId).filter((other) => NODE_BY_ID[other]!.kind !== 'hq' && topTroop(game, other)?.ownerId === playerId).length;
}

function holdsDepot(game: GameState, playerId: string): boolean {
  return game.rules.depots && CASTLE_MAP.depots.some((nodeId) => topTroop(game, nodeId)?.ownerId === playerId);
}
...
function knightDestinations(game: GameState, playerId: string, from: string): string[] {
  return neighbors(game, from).filter((to) => {
    ...
    return NODE_BY_ID[to]!.kind !== 'hq' && (!occupant || occupant.ownerId === playerId);
  });
}
...
function placements(game: GameState, playerId: string): Record<string, string[]> {
  return Object.fromEntries(player(game, playerId).hand.map((troop) => [
    troop.id, CASTLE_MAP.nodes.filter((node) => canPlaceTroop(game, playerId, troop, node.id)).map((node) => node.id),
  ]));
}
...
function awardMedals(game: GameState): void {
  for (const region of CASTLE_MAP.regions) {
    ...
    if (player(game, ownerId).medals >= CASTLE_MAP.medalTarget) {
...
function nextTurn(game: GameState): void {
  ...
  game.currentPlayerId = opponent(game, game.currentPlayerId).id;
  game.turn += 1;
...
function settle(game: GameState): void {
  ...
  while (game.deferredSpecials.length) {
    ...
    if (nodeIds.length) {
      game.pending = { type: 'recover', ...source, nodeIds };
      return;
    }
  }
  nextTurn(game);
...
function placeTroop(game: GameState, playerId: string, action: Extract<GameAction, { type: 'place' }>, rng: Random): void {
  ...
  if (node.kind === 'special') game.deferredSpecials.push(source);
  if (action.useAbility !== false) {
    switch (troop.type) {
      ...
      case 'robot': {
        ...
        if (enemy.hand.length) {
          ...
          addLog(game, playerId, `상대 손에서 ${withObjectParticle(TROOPS[removed.type].name)} 제거했습니다.`);
        }
        break;
      }
      case 'giant': {
      ...
      case 'sapper': {
        ...
        const nodeIds = ADJACENCY[node.id]!.filter((other) =>
          game.cutEdges.includes(edgeKey(node.id, other)) || (node.kind !== 'hq' && NODE_BY_ID[other]!.kind !== 'hq'));
        if (nodeIds.length) game.pending = { type: 'edge', ...source, nodeIds };
        break;
      }
      case 'knight': {
        const nodeIds = CASTLE_MAP.nodes.filter((candidate) => {
...
function chooseEffect(game: GameState, playerId: string, nodeId: string): void {
  ...
  if (pending.type === 'edge') {
    const key = edgeKey(pending.sourceNodeId, nodeId);
    const wasCut = game.cutEdges.includes(key);
    game.cutEdges = wasCut ? game.cutEdges.filter((other) => other !== key) : [...game.cutEdges, key];
    addLog(game, playerId, `${withAndParticle(NODE_BY_ID[pending.sourceNodeId]!.label)} ${NODE_BY_ID[nodeId]!.label} 사이 길을 ${wasCut ? '다시 이었습니다' : '끊었습니다'}.`);
    game.pending = null;
    settle(game);
    return;
  }
  ...
  if (pending.type === 'move-to') {
    ...
    addLog(game, playerId, `${withObjectParticle(TROOPS[mover.type].name)} ${withDirectionParticle(NODE_BY_ID[nodeId]!.label)} 옮겼습니다.`);
    game.pending = null;
    awardMedals(game);
    settle(game);
    return;
  ...
  if (pending.type === 'remove') {
    ...
    addLog(game, playerId, `${NODE_BY_ID[nodeId]!.label}의 ${withObjectParticle(TROOPS[target.type].name)} 제거했습니다.`);
  } else {
...
export function applyAction(original: GameState, playerId: string, action: GameAction, rng: Random = secureRandom): GameState {
  ...
  if (game.pending) {
    if (action.type === 'skip') {
      ...
      chooseEffect(game, playerId, action.nodeId);
    } else {
