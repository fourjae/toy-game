...
const SKILL_LABEL: Record<TroopType, string> = { duck: '만능 공격', skeleton: '2개 보충', captain: '추가 배치', giant: '인접 제거', pirate: '자유 배치', robot: '더미로 귀환', unicorn: '1개 보충', dino: '힘 7', ninja: '적진 잠입', sapper: '길 공사', knight: '아군 이동', bomb: '함정' };
...
function Rules({ onClose }: { onClose: () => void }) {
  ...
  return <Dialog title={tab === 'rules' ? '처음이라면, 이것만.' : '전장 둘러보기'} onClose={onClose} wide>
    ...
    <p className="rules-note">선공은 3개, 후공은 4개로 시작해요. 더는 뽑거나 놓을 수 없다면 훈장을 비교하며, 동점이면 행동하지 못한 쪽이 져요. 방장은 한 차례 시간을 30~90초 사이에서 5초 단위로 정할 수 있어요. 시간이 지나면 병정을 뽑고 차례가 넘어가고, 뽑을 수 없을 때는 가장 약한 병정이 알아서 놓여요.</p>
...
function ScorePlayer({ game, room, id, mine }: { game: GameView; room: RoomView; id: string; mine: boolean }) {
  ...
  return <div className={`score-player ${p.index === 0 ? 'blue-side' : 'red-side'} ${game.currentPlayerId === id ? 'is-turn' : ''}`}><div className={`avatar ${p.index === 0 ? 'blue' : 'red'}`}><ToyIcon type={p.index === 0 ? 'captain' : 'duck'} size={40} color={p.index === 0 ? 'blue' : 'red'} /></div><div className="score-player-name"><strong>{person?.name || '플레이어'}{mine && <small>나</small>}</strong><span>{person?.connected === false ? '다시 연결하는 중' : `${p.id === game.firstPlayerId ? '선공' : '후공'} · 병정 ${p.handCount}개`}</span></div><div className="score-medals"><Medal size={21} /><strong>{p.medals}<small>/ {getMap(game.mapId).medalTarget}</small></strong></div></div>;
}
...
function GameScreen({ room, meId, busy, send, onLeave, onRules }: { room: RoomView; meId: string; busy: boolean; send: (event: string, data?: object) => Promise<boolean>; onLeave: () => void; onRules: () => void }) {
  ...
  const hint = finished ? '이번 판이 끝났어요.' : !mine ? `${other?.name || '상대'}님이 생각하고 있어요.` : pending?.type === 'remove' ? '거인병 효과예요. 인접한 상대 병정 하나를 골라 버려요.' : pending?.type === 'recover' ? '북 거점 효과예요. 판 위의 내 병정 하나를 손으로 데려올 수 있어요.' : pending?.type === 'map-recover-discard' ? '묘지에서 버린 내 병정 하나를 손으로 데려와요.' : pending?.type === 'map-freeze-card' ? pending.cardEffect === 'return-to-supply' ? 'XB-42 효과예요. 상대의 뒷면 손패 하나를 골라 병정 더미로 돌려보내요.' : '상대의 뒷면 손패 하나를 골라 다음 차례에 묶어요.' : pending?.type === 'map-jungle-from' ? '용암으로 밀어낼 인접한 적 병정을 골라 주세요.' : pending?.type === 'map-jungle-to' ? '적 병정을 보낼 거점을 골라 주세요.' : pending?.type === 'extra-place' ? '캡틴 효과예요. 병정을 하나 더 놓을 수 있어요.' : pending?.type === 'edge' ? '공병 효과예요. 길 건너편 거점을 누르면 그 길을 끊거나 다시 이어요.' : pending?.type === 'move-from' ? '기사 효과예요. 옮길 내 병정을 골라 주세요.' : pending?.type === 'move-to' ? '어디로 옮길까요? 빛나는 거점을 눌러 주세요.' : selected ? legal.length ? '빛나는 거점에 병정을 놓아 주세요.' : '이 병정을 놓을 수 있는 곳이 없어요.' : '병정을 고르거나, 2개를 뽑아 주세요.';
