...
import { ArrowLeft, ArrowRight, Check, ChevronDown, ChevronRight, ChevronUp, Clock3, Copy, Flag, HelpCircle, History, Layers, LogOut, Medal, Plus, Shuffle, Sparkles, Timer, Users, WifiOff, X } from 'lucide-react';
...
import SupplyPile from './components/SupplyPile';
import { BASE_TROOP_TYPES, TROOPS, withObjectParticle } from '../shared/troops';
import { TURN_SECONDS } from '../shared/engine';
import type { GameEvent, GameRules, GameView, Troop, TroopType } from '../shared/types';
import { CASTLE_MAP, GAME_MAPS, getMap, nodeIndex } from '../shared/map';
...
const RULE_INFO: Record<keyof GameRules, { name: string; short: string; detail: string }> = {
  ...
  expansion: { name: '확장 병정', short: '확장 병정', detail: '그림자·공병·기사·폭탄이 각 2개씩 부대에 들어와요. 한 명당 병정이 32개가 돼요.' },
};
const RULE_KEYS = Object.keys(RULE_INFO) as (keyof GameRules)[];
const VISIBLE_RULE_KEYS: (keyof GameRules)[] = RULE_KEYS;
// The variant rules are parked until the user decides on them; the server ignores them unless ENABLE_EXPANSION_RULES=1.
const SHOW_RULE_OPTIONS = false;
const enabledRules = (rules: GameRules) => RULE_KEYS.filter(key => rules[key]);
const SKILL_LABEL: Record<TroopType, string> = { duck: '만능 공격', skeleton: '2개 보충', captain: '추가 배치', giant: '인접 제거', pirate: '자유 배치', robot: '손패 폐기', unicorn: '1개 보충', dino: '힘 7', ninja: '적진 잠입', sapper: '길 공사', knight: '아군 이동', bomb: '함정' };
const OPTIONAL_ABILITY: TroopType[] = ['skeleton', 'captain', 'giant', 'robot', 'unicorn', 'sapper', 'knight'];
const mapSelectionName = (selection: string) => selection === 'random' ? '랜덤 맵' : getMap(selection).name;
...
function MapPicker({ value, onChange, disabled = false }: { value: string; onChange: (value: string) => void; disabled?: boolean }) {
  return <label className="map-picker"><span><Layers size={16} /> 전장 선택</span><select value={value} disabled={disabled} onChange={event => onChange(event.target.value)}><option value="random">랜덤 맵 · {GAME_MAPS.length}개 중 하나</option>{GAME_MAPS.map(map => <option key={map.id} value={map.id}>{map.name} · 승리 조건 훈장 {map.medalTarget}개</option>)}</select><small>{value === 'random' ? '시작할 때 무작위로 정해져요. 재대전은 다시 뽑아요.' : `승리 조건 · 훈장 ${getMap(value).medalTarget}개 또는 상대 본부 점령 · ${getMap(value).specialDescription}`}</small></label>;
}

function TurnTimeSlider({ value, onCommit, disabled = false }: { value: number; onCommit: (value: number) => void; disabled?: boolean }) {
  ...
  const progress = (draft - 30) / 60 * 100;
  return <div className={`turn-time-control ${disabled ? 'is-readonly' : ''}`}>
    <div className="turn-time-heading"><span><Shuffle size={15} /> 선공은 무작위</span><strong><Timer size={16} /> 한 차례 <b>{draft}초</b></strong></div>
    <input aria-label="한 차례 제한 시간" type="range" min="30" max="90" step="5" value={draft} disabled={disabled} style={{ '--range-progress': `${progress}%` } as React.CSSProperties} onChange={event => setDraft(Number(event.target.value))} onPointerUp={event => commit(event.currentTarget)} onKeyUp={event => commit(event.currentTarget)} onBlur={event => commit(event.currentTarget)} />
    <div className="turn-time-scale"><span>30초</span><span>5초 단위</span><span>90초</span></div>
  </div>;
}
...
function FieldPreview({ label = true, mapId = CASTLE_MAP.id, random = false }: { label?: boolean; mapId?: string; random?: boolean }) {
  const map = getMap(mapId);
  return <div className="field-preview"><div className="preview-top"><span><i className="status-dot" /> 오늘의 전장</span><span>{random ? '랜덤 · 시작할 때 공개' : `${String(GAME_MAPS.findIndex(item => item.id === map.id) + 1).padStart(2, '0')} / ${map.name}`}</span></div>{random ? <MysteryField /> : <Board preview previewMapId={map.id} />}{label && <div className="preview-bottom"><span className="preview-copy"><span><Flag size={16} /> {random ? `시작하면 ${GAME_MAPS.length}개 중 하나가 펼쳐져요.` : map.description}</span>{!random && <small><Sparkles size={13} /> 승리 조건 · 훈장 {map.medalTarget}개 또는 상대 본부 점령 · 땅 효과 · {map.specialDescription}</small>}</span><span className="handwritten">{random ? '어디가 나올까?' : '어느 쪽으로 갈까?'}</span></div>}</div>;
}

/** Stands in for the board while the map is still a secret. */
function MysteryField() {
  return <div className="mystery-field" role="img" aria-label="아직 정해지지 않은 전장"><div className="mystery-mark">?</div><div className="mystery-chips">{GAME_MAPS.map((map, index) => <span key={map.id} className={`map-guide-number theme-${map.theme}`}>{String(index + 1).padStart(2, '0')}</span>)}</div></div>;
}

function Rules({ onClose }: { onClose: () => void }) {
  const [tab, setTab] = useState<'rules' | 'maps'>('rules');
  return <Dialog title={tab === 'rules' ? '처음이라면, 이것만.' : '전장 둘러보기'} onClose={onClose} wide>
    <div className="rules-tabs" role="tablist"><button type="button" role="tab" aria-selected={tab === 'rules'} className={tab === 'rules' ? 'active' : ''} onClick={() => setTab('rules')}><HelpCircle size={15} /> 게임 방법</button><button type="button" role="tab" aria-selected={tab === 'maps'} className={tab === 'maps' ? 'active' : ''} onClick={() => setTab('maps')}><Layers size={15} /> 맵 둘러보기 <span>{GAME_MAPS.length}</span></button></div>
    {tab === 'maps' ? <div role="tabpanel"><p className="dialog-copy">방을 만들 때나 대기실에서 방장이 랜덤 또는 지정 맵을 고를 수 있어요. 특수 거점은 노란 테두리로 표시돼요.</p><MapGallery /><p className="rules-note">전장 {GAME_MAPS.length}개의 배치는 온라인 대전에 맞춰 재구성한 버전입니다. 특수 거점의 효과도 대전에서 직관적으로 작동하도록 조정했어요.</p></div> : <div role="tabpanel">
    <div className="rules-intro"><span className="eyebrow">토이 배틀 · 2인 · 전장 {GAME_MAPS.length}개</span><p>상대 성에 먼저 도착하거나,<br /><strong>전장의 목표 훈장</strong>을 모으면 승리해요.</p></div>
    ...
    <h3 className="rules-section-title">전장 선택 <small>방장에게 랜덤 또는 지정 맵 선택권이 있어요</small></h3>
    <p className="rules-note">전장 {GAME_MAPS.length}개는 위의 맵 둘러보기 탭에서 길과 특수 거점까지 한눈에 볼 수 있어요.</p>
    ...
    <p className="rules-note">각자 병정 24개를 전부 섞어 병정 더미를 만들고, 선공은 3개, 후공은 4개로 시작해요. 제거된 병정은 둘이 함께 쓰는 공용 버림 더미로 갑니다. 더는 뽑거나 놓을 수 없다면 훈장을 비교하며, 동점이면 행동하지 못한 쪽이 져요. 방장은 한 차례 시간을 30~90초 사이에서 5초 단위로 정할 수 있어요. 시간이 지나면 병정을 뽑고 차례가 넘어가고, 뽑을 수 없을 때는 가장 약한 병정이 알아서 놓여요.</p>
...
function ScorePlayer({ game, room, id, mine }: { game: GameView; room: RoomView; id: string; mine: boolean }) {
  ...
  return <div className={`score-player ${p.index === 0 ? 'blue-side' : 'red-side'} ${game.currentPlayerId === id ? 'is-turn' : ''}`}><div className={`avatar ${p.index === 0 ? 'blue' : 'red'}`}><ToyIcon type={p.index === 0 ? 'captain' : 'duck'} size={40} color={p.index === 0 ? 'blue' : 'red'} /></div><div className="score-player-name"><strong>{person?.name || '플레이어'}{mine && <small>나</small>}</strong><span>{person?.connected === false ? '다시 연결하는 중' : `${p.id === game.firstPlayerId ? '선공' : '후공'} · 손패 ${p.handCount}개`}</span></div><SupplyPile count={p.supplyCount} color={p.index === 0 ? 'blue' : 'red'} owner={mine ? '내' : '상대'} /><div className="score-medals"><Medal size={21} /><strong>{p.medals}<small>/ {getMap(game.mapId).medalTarget}</small></strong></div></div>;
}
...
function initialOrientation(): BoardOrientation {
  // A phone always opens with the field standing upright. Desktop orientation
  // preferences must not leak into a narrow/mobile session on the same device.
  if (window.matchMedia('(max-width: 640px)').matches) return 'portrait';
  try {
...
type HandPhase = 'entering' | 'returned' | 'stay' | 'leaving' | 'decking';
 ...
 */
function useAnimatedHand(hand: Troop[], discarded: Troop[], events: GameEvent[]) {
  const [cards, setCards] = useState<HandCard[]>([]);
  const counter = useRef(0);
  useEffect(() => {
    const discardedIds = new Set(discarded.map(t => t.id));
    const returnedIds = new Set(events.filter(event => event.type === 'return-to-hand').map(event => event.troop?.id));
    const deckedIds = new Set(events.filter(event => event.type === 'return-to-supply').map(event => event.troop?.id));
    setCards(previous => {
      ...
      for (const card of previous) {
        ...
        else if (card.phase === 'leaving' || card.phase === 'decking') next.push(card);
        else if (discardedIds.has(card.troop.id)) next.push({ ...card, phase: 'leaving' });
        else if (deckedIds.has(card.troop.id)) next.push({ ...card, phase: 'decking' });
      }
      for (const troop of hand) if (!known.has(troop.id)) next.push({ troop, phase: returnedIds.has(troop.id) ? 'returned' : 'entering', order: counter.current++ });
      return next;
    });
  }, [hand, discarded, events]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setCards(current => current.filter(c => c.phase !== 'leaving' && c.phase !== 'decking').map(c => c.phase === 'entering' || c.phase === 'returned' ? { ...c, phase: 'stay' } : c)), 1400);
    return () => clearTimeout(timer);
  }, [cards]);
  const settle = useCallback((troopId: string) => {
    setCards(current => current.flatMap(c => c.troop.id !== troopId ? [c] : c.phase === 'leaving' || c.phase === 'decking' ? [] : [{ ...c, phase: 'stay' }]));
  }, []);
  return { cards, settle };
}
...
function GameScreen({ room, meId, busy, send, onLeave, onRules }: { room: RoomView; meId: string; busy: boolean; send: (event: string, data?: object) => Promise<boolean>; onLeave: () => void; onRules: () => void }) {
  const game = room.game!;
  const map = getMap(game.mapId);
  const [selected, setSelected] = useState<string | null>(null);
  const [useAbility, setUseAbility] = useState(true);
  const [showLog, setShowLog] = useState(false);
  // On a phone the hand dock is fixed to the bottom; the board takes whatever height is left above it.
  const dockRef = useRef<HTMLElement>(null);
  const [dockHeight, setDockHeight] = useState(240);
  useEffect(() => {
    const dock = dockRef.current;
    if (!dock) return;
    const observer = new ResizeObserver(() => setDockHeight(Math.round(dock.getBoundingClientRect().height)));
  ...
  useEffect(() => {
    ...
    const keepMobilePortrait = () => { if (mobile.matches) setOrientation('portrait'); };
    keepMobilePortrait();
    mobile.addEventListener('change', keepMobilePortrait);
    return () => mobile.removeEventListener('change', keepMobilePortrait);
  }, []);
  ...
  const { cards: handCards, settle: settleCard } = useAnimatedHand(game.hand, game.discarded, game.events);
  ...
  async function place(nodeId: string) {
    if (!mine || busy || finished) return;
    if (!legal.includes(nodeId)) {
      if (pending && pending.type !== 'extra-place') { await send('game:action', { type: 'skip' }); return; }
      if (game.board[nodeId]?.length) setInspect(nodeId);
      return;
    }
  ...
  function cancelPending() {
    if (mine && !busy && !finished && pending && pending.type !== 'extra-place') void send('game:action', { type: 'skip' });
  }
  const hint = finished ? '이번 판이 끝났어요.' : !mine ? `${other?.name || '상대'}님이 생각하고 있어요.` : pending?.type === 'remove' ? '거인병 효과예요. 인접한 상대 병정 하나를 골라 버려요.' : pending?.type === 'recover' ? '북 거점 효과예요. 판 위의 내 병정 하나를 손으로 데려올 수 있어요.' : pending?.type === 'map-recover-discard' ? '묘지에서 버린 내 병정 하나를 손으로 데려와요.' : pending?.type === 'map-freeze-card' ? pending.cardEffect === 'discard' ? 'XB-42 효과예요. 상대의 뒷면 손패 하나를 골라 공용 버림 더미로 보내요.' : '상대의 뒷면 손패 하나를 골라 다음 차례에 묶어요.' : pending?.type === 'map-jungle-from' ? '용암으로 밀어낼 인접한 적 병정을 골라 주세요.' : pending?.type === 'map-jungle-to' ? '적 병정을 보낼 거점을 골라 주세요.' : pending?.type === 'extra-place' ? '캡틴 효과예요. 병정을 하나 더 놓을 수 있어요.' : pending?.type === 'edge' ? '공병 효과예요. 길 건너편 거점을 누르면 그 길을 끊거나 다시 이어요.' : pending?.type === 'move-from' ? '기사 효과예요. 옮길 내 병정을 골라 주세요.' : pending?.type === 'move-to' ? '어디로 옮길까요? 빛나는 거점을 눌러 주세요.' : selected ? legal.length ? '빛나는 거점에 병정을 놓아 주세요.' : '이 병정을 놓을 수 있는 곳이 없어요.' : '병정을 고르거나, 2개를 뽑아 주세요.';
  ...
  const topbarSide = (id: string) => {
    const p = game.players.find(candidate => candidate.id === id)!;
    const person = room.players.find(candidate => candidate.id === id);
    return <div className={`topbar-side ${p.index === 0 ? 'blue' : 'red'} ${game.currentPlayerId === id && !finished ? 'is-turn' : ''} ${person?.connected === false ? 'is-away' : ''}`}><ToyIcon type={p.index === 0 ? 'captain' : 'duck'} size={26} color={p.index === 0 ? 'blue' : 'red'} /><span className="topbar-name">{person?.name || '플레이어'}{id === meId && <small>나</small>}</span><span className="topbar-hand" title="손에 든 병정"><Layers size={11} />{p.handCount}</span><SupplyPile compact count={p.supplyCount} color={p.index === 0 ? 'blue' : 'red'} owner={id === meId ? '내' : '상대'} /><span className="topbar-medals" title="훈장"><Medal size={12} />{p.medals}</span></div>;
  };
  return <main className={`game-screen page-width ${handCollapsed ? 'hand-collapsed' : ''}`} style={{ '--dock-h': `${dockHeight}px` } as React.CSSProperties}><header className="game-topbar"><button type="button" className="topbar-btn" onClick={onLeave} aria-label="방 나가기"><ArrowLeft size={18} /></button><div className="topbar-score">{topbarSide(game.players[0].id)}<span className="topbar-vs"><strong>vs</strong><small>{finished ? '종료' : `${game.turn}번째`}</small></span>{topbarSide(game.players[1].id)}</div><button type="button" className="topbar-btn" onClick={() => setShowLog(!showLog)} aria-label="지난 차례 기록"><History size={18} /></button><button type="button" className="topbar-btn" onClick={onRules} aria-label="게임 방법"><HelpCircle size={18} /></button></header><div className="game-title-row"><button className="text-button" onClick={onLeave}><ArrowLeft size={16} /> 방 나가기</button><span>{room.title}<span className="room-code-small">{room.code}</span></span><button className="text-button log-toggle" onClick={() => setShowLog(!showLog)}>지난 차례</button></div><div className="scoreboard"><ScorePlayer game={game} room={room} id={game.players[0].id} mine={game.players[0].id === meId} /><div className="versus"><span>{map.name}</span><strong>vs.</strong><span>{game.turn}번째 차례</span></div><ScorePlayer game={game} room={room} id={game.players[1].id} mine={game.players[1].id === meId} /></div><div className="game-layout"><section className="board-wrap"><div className="board-heading"><span><Flag size={15} /> {map.name} <small>{room.mapSelection === "random" ? "랜덤맵" : "지정맵"}</small><RuleBadges rules={game.rules} /></span><span><Medal size={15} /> 훈장 {map.medalTarget}개 또는 상대 본부 점령</span></div><div className="board-stage"><OpponentHand cards={game.opponentHand} color={ownPlayer.index === 0 ? 'red' : 'blue'} choosing={pending?.type === 'map-freeze-card'} selectableSlots={pending?.type === 'map-freeze-card' ? pending.troopIds ?? [] : []} busy={busy || finished} onChoose={slot => void send('game:action', { type: 'choose-card', troopId: slot })} discarded={game.discarded} discardedColor={game.players.find(player => player.id === game.discarded.at(-1)?.ownerId)?.index === 1 ? 'red' : 'blue'} /><Board game={game} legalNodes={legal} onNodeClick={place} onBackgroundClick={cancelPending} selectedNodeId={pending?.sourceNodeId} orientation={orientation} bottomIndex={ownPlayer.index} onToggleOrientation={toggleOrientation} highlightEdges={roadChoices} />{events.length > 0 && <div className="board-events" aria-live="assertive">{events.map(event => <div key={event.id} className={`board-event ${event.mine ? 'is-loss' : 'is-gain'}`}><span className="board-event-icon"><ToyIcon type={event.troop.type} size={46} color={game.players[0].id === event.troop.ownerId ? 'blue' : 'red'} /><X size={14} /></span><p><strong>{event.mine ? '병정을 잃었어요' : '상대 병정을 버렸어요'}</strong><span>{event.text}</span></p></div>)}</div>}</div>{turnBar('turn-bar--board')}</section><aside className={`game-sidebar ${showLog ? 'show-log' : ''}`}><div className="sidebar-title"><h3>지난 차례</h3><button className="icon-button log-close" aria-label="기록 닫기" onClick={() => setShowLog(false)}><X size={18} /></button><span>기록</span></div><div className="game-log" aria-live="polite">{[...game.log].reverse().map(entry => <div key={entry.id}><span className={game.players[0].id === entry.playerId ? 'blue-dot' : 'red-dot'} /><p><strong>{room.players.find(p => p.id === entry.playerId)?.name || '플레이어'}</strong><span>{entry.text}</span></p><small>{entry.turn}</small></div>)}</div><div className="sidebar-tip"><Flag size={22} /><p>성으로 가는 길을<br />차근차근 이어 보세요.</p><span>길이 끊기면 더 나아갈 수 없어요.</span></div></aside></div>
    <section ref={dockRef} className={`hand-dock ${!mine ? 'waiting-turn' : ''} ${handCollapsed ? 'is-collapsed' : ''}`}>
      {turnBar('turn-bar--dock')}
      <button type="button" className="hand-mobile-toggle" onClick={() => setHandCollapsed(!handCollapsed)} aria-expanded={!handCollapsed} aria-label={handCollapsed ? `패 펼치기, ${game.hand.length}장` : '패 접기'}>{handCollapsed ? <ChevronUp size={15} /> : <ChevronDown size={15} />}<span>{handCollapsed ? `패 펼치기 · ${game.hand.length}장` : '패 접기'}</span></button>
      <div className="hand-topline"><h3><button type="button" className="hand-toggle" onClick={() => setHandCollapsed(!handCollapsed)} aria-expanded={!handCollapsed} aria-label={handCollapsed ? '손패 펼치기' : '손패 접기'}>{handCollapsed ? <ChevronUp size={15} /> : <ChevronDown size={15} />}</button>내 손 <span>병정 {game.hand.length} / 8</span>{handCollapsed && definition && <em className="hand-selected-note">{definition.name} 선택 중</em>}</h3><div className="hand-action">{pending ? <button className="button secondary compact" disabled={busy || finished} onClick={() => send('game:action', { type: 'skip' })}>효과 건너뛰기 <ChevronRight size={15} /></button> : <button className="button primary compact" disabled={!game.canDraw || busy || finished} onClick={() => send('game:action', { type: 'draw' })}><Plus size={17} /> {drawCount > 0 ? `병정 ${drawCount}개 뽑기` : ownPlayer.supplyCount ? '손이 가득 찼어요' : '뽑을 병정이 없어요'}</button>}<span className="supply-count">남은 병정 {ownPlayer.supplyCount}개</span></div></div>
      <div className="hand-cards">{handCards.map((card, i) => { const t = card.troop; const power = TROOPS[t.type].power; const frozen = game.frozenTroopIds.includes(t.id); const phaseClass = card.phase === 'entering' ? 'is-entering' : card.phase === 'returned' ? 'is-returned' : card.phase === 'leaving' ? 'is-leaving' : card.phase === 'decking' ? 'is-decking' : ''; return <button className={`troop-card ${ownPlayer.index === 0 ? 'blue-card' : 'red-card'} ${selected === t.id ? 'selected' : ''} ${phaseClass} ${frozen ? 'is-frozen' : ''}`} key={t.id} aria-label={`${TROOPS[t.type].name}, 힘 ${power || '조커'}${frozen ? ', 이번 차례 배치 불가' : ''}`} aria-pressed={selected === t.id} aria-hidden={card.phase === 'leaving' || card.phase === 'decking' || undefined} style={{ '--card-index': i, '--enter-index': card.phase === 'entering' ? i - enteringStart : 0 } as React.CSSProperties} onAnimationEnd={e => { if (e.target === e.currentTarget && ['card-deal', 'card-collapse', 'card-return'].includes(e.animationName)) settleCard(t.id); }} onClick={() => { setSelected(selected === t.id ? null : t.id); setUseAbility(true); }} disabled={busy || finished || card.phase === 'leaving' || card.phase === 'decking' || frozen}><span className="troop-power">{power || '★'}</span><ToyIcon type={t.type} size={65} color={ownPlayer.index === 0 ? 'blue' : 'red'} /><strong>{TROOPS[t.type].name}</strong><span className="troop-skill">{SKILL_LABEL[t.type]}</span></button>; })}
        {handCards.length === 0 && <p className="empty-hand">손이 비었어요. 병정을 뽑아 주세요.</p>}<div className="hand-end"><span>한 수,<br />신중하게.</span><span>↙</span></div></div>
      <div className="selected-description">{definition ? <><strong>{definition.name}</strong><span>{definition.ability}</span>{OPTIONAL_ABILITY.includes(troop!.type) && mine && <label><input type="checkbox" checked={useAbility} onChange={e => setUseAbility(e.target.checked)} /> 능력 사용</label>}</> : <span>{mine ? '병정을 누르면 능력과 놓을 수 있는 곳이 보여요.' : '내 병정을 눌러 다음 수를 미리 생각해 보세요.'}</span>}</div>
    </section>
    {pending?.type === 'map-recover-discard' && <Dialog title="묘지에서 병정 회수" onClose={() => void send('game:action', { type: 'skip' })}><p className="muted">손으로 가져올 내 병정을 하나 골라 주세요.</p><div className="stack-list">{game.discarded.filter(t => pending.troopIds?.includes(t.id)).map(t => <button key={t.id} onClick={() => void send('game:action', { type: 'choose-card', troopId: t.id })}><ToyIcon type={t.type} size={44} color={ownPlayer.index === 0 ? 'blue' : 'red'} /><span>{TROOPS[t.type].name}<small>힘 {TROOPS[t.type].power}</small></span><ChevronRight size={16} /></button>)}</div></Dialog>}
    {finished && !resultHidden && <Dialog title={game.winnerId === meId ? '이번 판은, 내 승리!' : '좋은 승부였어요.'} onClose={() => setResultHidden(true)}><div className="result-content"><div className={`result-medal ${game.winnerId === meId ? '' : 'lost'}`}><Medal size={58} /></div><h3>{room.players.find(p => p.id === game.winnerId)?.name}님의 승리</h3><p>{game.winReason === 'headquarters' ? '상대 본부를 점령했어요.' : game.winReason === 'medals' ? `훈장 ${map.medalTarget}개를 먼저 모았어요.` : game.winReason === 'forfeit' ? '상대가 게임을 떠났어요.' : '더는 놓을 병정이 없어 훈장으로 승부를 정했어요.'}</p><div className="result-score">{room.players.map(p => <span key={p.id}>{p.name}<strong>{game.players.find(gp => gp.id === p.id)?.medals}<small>훈장</small></strong></span>)}</div><button className="button primary full" disabled={busy || room.rematchVotes.includes(meId) || !other?.connected} onClick={() => send('game:rematch')}>{room.rematchVotes.includes(meId) ? '친구의 답을 기다리는 중' : room.rematchVotes.length ? '좋아요, 한 판 더!' : '한 판 더 하기'}<Shuffle size={17} /></button><button className="text-button result-leave" onClick={onLeave}>대기실로 나가기 <ArrowRight size={15} /></button></div></Dialog>}
...
export default function App() {
  ...
  const [roomMapSelection, setRoomMapSelection] = useState('random');
  const [roomTurnSeconds, setRoomTurnSeconds] = useState(TURN_SECONDS);
  ...
  async function createRoom(e: FormEvent) { e.preventDefault(); if (await send('rooms:create', { title: roomTitle.trim() || `${me!.name}님의 방`, rules: roomRules, mapSelection: roomMapSelection, turnSeconds: roomTurnSeconds })) { setCreating(false); setRoomTitle(''); } }
  ...
  return <div className={`app ${isPlaying ? 'in-game' : ''}`}><header className="site-header page-width"><Brand /><nav>{me && <span className="header-player"><span className="status-dot" />{me.name}<small>님</small></span>}<button className="text-button rules-button" onClick={() => setRules(true)}><HelpCircle size={17} /><span>게임 방법</span></button>{me && !room && <button className="icon-button logout-button" onClick={client.logout} aria-label="이름 바꾸기" title="이름 바꾸기"><LogOut size={18} /></button>}</nav></header>{replaced ? <div className="connection-banner" role="status"><WifiOff size={15} /> 다른 탭에서 접속해서 이 탭은 잠시 쉬고 있어요.<button className="banner-button" onClick={client.reconnect}>여기서 계속하기</button></div> : !connected && <div className="connection-banner" role="status"><WifiOff size={15} /> 서버에 연결하는 중이에요. 잠시만 기다려 주세요.</div>}
    {restoring ? <main className="loading-page"><span className="loading-dice">⚄</span><p>자리를 찾고 있어요.</p></main> : !me ? <Login onEnter={name => void send('session:enter', { name })} busy={busy} connected={connected} onRules={() => setRules(true)} invite={invite.current} /> : room?.game ? <GameScreen room={room} meId={me.id} busy={busy || !connected} send={send} onLeave={() => setLeaving(true)} onRules={() => setRules(true)} /> : room ? <main className="waiting-page page-width"><div className="page-topline"><button className="text-button" onClick={() => void leaveRoom()}><ArrowLeft size={16} /> 대기실로</button><span className="eyebrow">ROOM {room.code}</span></div><div className="waiting-heading"><span className="eyebrow">자, 자리를 잡아 볼까요?</span><h1>{room.title}</h1><p>{room.players.length === 2 ? '둘 다 모였네요. 이제 시작해 볼까요?' : '한 자리 비워 뒀어요. 같이 놀 친구를 불러 주세요.'}</p><p className="waiting-rules">{enabledRules(room.rules).length ? <>이 방의 규칙 <RuleBadges rules={room.rules} /></> : '기본 규칙으로 해요.'}</p></div><div className="waiting-grid">
<FieldPreview mapId={room.mapSelection === 'random' ? CASTLE_MAP.id : room.mapSelection} random={room.mapSelection === 'random'} />
<section className="waiting-panel"><div className="panel-label"><h2>오늘의 플레이어</h2><span>{room.players.length} / 2</span></div>{[0, 1].map(i => { const p = room.players[i]; return <PlayerSlot key={i} index={i} name={p?.name} mine={p?.id === me.id} host={p?.isHost} connected={p?.connected} />; })}<div className="invite-box"><div><span>친구에게 이 코드를 알려 주세요</span><strong>{room.code}</strong></div><button className="button secondary compact" onClick={copyInvite}>{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? '복사했어요' : '초대 링크'}</button></div><MapPicker value={room.mapSelection} disabled={room.hostId !== me.id || busy || !connected} onChange={value => void send('rooms:map', { mapSelection: value })} /><div className="start-area"><TurnTimeSlider value={room.turnSeconds} disabled={room.hostId !== me.id || busy || !connected} onCommit={value => void send('rooms:turn-time', { turnSeconds: value })} /><button className="button primary full" disabled={busy || !connected || room.players.length !== 2 || room.players.some(p => !p.connected) || room.hostId !== me.id} onClick={() => send('game:start')}>{room.hostId !== me.id ? '방장이 시작하기를 기다리는 중' : room.players.length < 2 ? '친구가 오면 시작할 수 있어요' : '게임 시작'}<ArrowRight size={19} /></button></div>{room.notice && <p className="room-notice">{room.notice}</p>}</section></div></main> : <main className="lobby page-width"><div className="lobby-heading"><div><div className="eyebrow"><span className="status-dot" /> 플레이어 대기실</div><h1>반가워요, {me.name}님<span className="blue-punctuation">.</span></h1><p>빈자리에 앉거나, 새 판을 펼쳐 보세요.</p></div><button className="button primary" onClick={() => setCreating(true)}><Plus size={19} /> 방 만들기</button></div><div className="lobby-grid"><section className="lobby-field"><FieldPreview /><div className="lobby-field-note"><div><span className="eyebrow">THE CLASSIC</span><h2>성채 평원</h2><p>첫 수는 가볍게, 마지막 수는 신중하게.</p></div><button className="round-arrow" onClick={() => setRules(true)} aria-label="성채 평원 게임 방법"><ArrowRight size={23} /></button></div><div className="field-facts"><span><Users size={16} /> 2인 대전</span><span><Clock3 size={16} /> 약 15분</span><span><Medal size={16} /> 목표 훈장 7개</span><span><Timer size={16} /> 차례당 30~90초</span></div></section><section className="room-list-panel"><div className="room-list-top"><h2>열린 방 <span>{rooms.length}</span></h2><button className="text-button" onClick={() => setJoining(true)}>코드로 입장 <ArrowRight size={15} /></button></div><div className="room-filters"><button className={filter === 'waiting' ? 'active' : ''} onClick={() => setFilter('waiting')}>기다리는 방</button><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>모든 방</button></div><div className="room-list">{rooms.filter(r => filter === 'all' || r.canJoin).map(r => <button key={r.id} className="room-row" disabled={!r.canJoin || busy} onClick={() => send('rooms:join', { id: r.id })}><span className={`room-icon ${r.canJoin ? '' : 'room-in-play'}`}><Flag size={22} /></span><span className="room-row-name"><strong>{r.title}</strong><span>{r.hostName} · {mapSelectionName(r.mapSelection)} · {r.turnSeconds}초<RuleBadges rules={r.rules} className="rule-badges--row" />
</span></span><span className="room-row-right"><span className={r.canJoin ? 'open-badge' : 'playing-badge'}>{r.canJoin ? '참가 가능' : r.phase === 'playing' ? '게임 중' : '자리 없음'}</span><span><Users size={14} />{r.playerCount}/2</span></span>{r.canJoin && <ChevronRight size={17} />}</button>)}{rooms.filter(r => filter === 'all' || r.canJoin).length === 0 && <div className="empty-rooms"><div className="empty-illustration"><ToyIcon type="duck" color="blue" size={78} /><span>…</span></div><h3>{filter === 'all' ? '아직 펼쳐진 판이 없어요' : '아직 기다리는 방이 없어요'}</h3><p>첫 번째 방의 주인이 되어 보세요.<br />초대 링크로 친구를 부를 수 있어요.</p><button className="text-link" onClick={() => setCreating(true)}>우리 방 만들기 <ArrowRight size={16} /></button></div>}</div><div className="room-list-foot"><span className="status-dot" /> 방 목록은 실시간으로 바뀌어요</div></section></div></main>}
