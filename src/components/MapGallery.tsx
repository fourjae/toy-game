...
export default function MapGallery() {
  return <div className="map-gallery">
    {GAME_MAPS.map((map, index) => <div key={map.id} className="map-card" data-map-id={map.id}>
      ...
      <div className="map-card-text"><strong><span className={`map-guide-number theme-${map.theme}`}>{String(index + 1).padStart(2, '0')}</span>{map.name}</strong><small>{map.description}</small><em>{map.specialDescription}</em></div>
    </div>)}
  </div>;
}
